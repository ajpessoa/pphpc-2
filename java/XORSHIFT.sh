...
mkdir XORSHIFT-Results

for configuration in "${configurations[@]}"
  do
    for threads in "${thread_counts[@]}"
    do
      echo "Running XORSHIFT with configuration $configuration and $threads threads"
      for (( i=1; i<=$runs; i++ ))
      do
        name="XORSHIFT_$configuration-$threads-$i.txt"
        sh ./pp.sh -p ../configs/$configuration.txt -ps EX -n $threads -g XORSHIFT -s XORSHIFT-Results/$name
      done
    done
  done