...
mkdir REALLYPOOR-Results

for configuration in "${configurations[@]}"
  do
    for threads in "${thread_counts[@]}"
    do
      echo "Running MIDMODSQUARE (REALLYPOOR) with configuration $configuration and $threads threads"
      for (( i=1; i<=$runs; i++ ))
      do
        name="REALLYPOOR$configuration-$threads-$i.txt"
        sh ./pp.sh -p ../configs/$configuration.txt -ps EX -n $threads -g REALLYPOOR -s REALLYPOOR-Results/$name
      done
    done
  done
