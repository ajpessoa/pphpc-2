...
for configuration in "${configurations[@]}"
  do
    ...
    do
      echo "Running CA with configuration $configuration and $threads threads"
      for (( i=1; i<=$runs; i++ ))
      do
        name="CA_$configuration-$threads-$i.txt"
        sh ./pp.sh -p ../configs/$configuration.txt -ps EX -n $threads -g CA -s CA-Results/$name
      done
    done
  done