...
mkdir ACWELL-Results

for configuration in "${configurations[@]}"
  do
    for threads in "${thread_counts[@]}"
    do
      echo "Running ACWELL with configuration $configuration and $threads threads"
      for (( i=1; i<=$runs; i++ ))
      do
        name="ACWELL_$configuration-$threads-$i.txt"
        sh ./pp.sh -p ../configs/$configuration.txt -ps EX -n $threads -g ACWELL -s ACWELL-Results/$name
      done
    done
  done