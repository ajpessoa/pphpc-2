...
mkdir BETTERXOR-Results

for configuration in "${configurations[@]}"
  do
    for threads in "${thread_counts[@]}"
    do
      echo "Running BETTERXOR with configuration $configuration and $threads threads"
      for (( i=1; i<=$runs; i++ ))
      do
        name="BETTERXOR_$configuration-$threads-$i.txt"
        sh ./pp.sh -p ../configs/$configuration.txt -ps EX -n $threads -g BETTERXOR -s BETTERXOR-Results/$name
      done
    done
  done