...
mkdir PCG128-Results

for configuration in "${configurations[@]}"
  do
    for threads in "${thread_counts[@]}"
    do
      echo "Running PCG128 with configuration $configuration and $threads threads"
      for (( i=1; i<=$runs; i++ ))
      do
        name="PCG128_$configuration-$threads-$i.txt"
        sh ./pp.sh -p ../configs/$configuration.txt -ps EX -n $threads -g PCG128 -s PCG128-Results/$name
      done
    done
  done