...
for configuration in "${configurations[@]}"
  do
    ...
    do
      echo "Running CMWC with configuration $configuration and $threads threads"
      for (( i=1; i<=$runs; i++ ))
      do
        name="CMWC_$configuration-$threads-$i.txt"
        sh ./pp.sh -p ../configs/$configuration.txt -ps EX -n $threads -g CMWC -s CMWC-Results/$name
      done
    done
  done