...
for configuration in "${configurations[@]}"
  do
    ...
    do
      echo "Running RANDU with configuration $configuration and $threads threads"
      for (( i=1; i<=$runs; i++ ))
      do
        name="RANDU_$configuration-$threads-$i.txt"
        sh ./pp.sh -p ../configs/$configuration.txt -ps EX -n $threads -g RANDU -s RANDU-Results/$name
      done
    done
  done