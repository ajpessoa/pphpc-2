...
for configuration in "${configurations[@]}"
  do
    ...
    do
      echo "Running MT with configuration $configuration and $threads threads"
      for (( i=1; i<=$runs; i++ ))
      do
        name="MT_$configuration-$threads-$i.txt"
        sh ./pp.sh -p ../configs/$configuration.txt -ps EX -n $threads -g MT -s MT-Results/$name
      done
    done
  done