...
mkdir BETTERCMWC-Results

for configuration in "${configurations[@]}"
  do
    for threads in "${thread_counts[@]}"
    do
      echo "Running BETTERCMWC with configuration $configuration and $threads threads"
      for (( i=1; i<=$runs; i++ ))
      do
        name="BETTERCMWC_$configuration-$threads-$i.txt"
        sh ./pp.sh -p ../configs/$configuration.txt -ps EX -n $threads -g BETTERCMWC -s BETTERCMWC-Results/$name
      done
    done
  done