...
mkdir PRNGLCG-Results

for configuration in "${configurations[@]}"
  do
    for threads in "${thread_counts[@]}"
    do
      echo "Running PRNGLCG with configuration $configuration and $threads threads"
      for (( i=1; i<=$runs; i++ ))
      do
        name="PRNGLCG_$configuration-$threads-$i.txt"
        sh ./pp.sh -p ../configs/$configuration.txt -ps EX -n $threads -g PRNGLCG -s PRNGLCG-Results/$name
      done
    done
  done