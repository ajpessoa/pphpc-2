...
mkdir ACXORSHIFT-Results

for configuration in "${configurations[@]}"
  do
    for threads in "${thread_counts[@]}"
    do
      echo "Running ACXORSHIFT with configuration $configuration and $threads threads"
      for (( i=1; i<=$runs; i++ ))
      do
        name="ACXORSHIFT_$configuration-$threads-$i.txt"
        sh ./pp.sh -p ../configs/$configuration.txt -ps EX -n $threads -g ACXORSHIFT -s ACXORSHIFT-Results/$name
      done
    done
  done