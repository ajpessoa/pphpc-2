...
mkdir ACMT64-Results

for configuration in "${configurations[@]}"
  do
    for threads in "${thread_counts[@]}"
    do
      echo "Running ACMT64 with configuration $configuration and $threads threads"
      for (( i=1; i<=$runs; i++ ))
      do
        name="ACMT64_$configuration-$threads-$i.txt"
        sh ./pp.sh -p ../configs/$configuration.txt -ps EX -n $threads -g ACMT64 -s ACMT64-Results/$name
      done
    done
  done