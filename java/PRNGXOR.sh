...
mkdir PRNGXOR-Results

for configuration in "${configurations[@]}"
  do
    for threads in "${thread_counts[@]}"
    do
      echo "Running PRNGXOR with configuration $configuration and $threads threads"
      for (( i=1; i<=$runs; i++ ))
      do
        name="PRNGXOR_$configuration-$threads-$i.txt"
        sh ./pp.sh -p ../configs/$configuration.txt -ps EX -n $threads -g PRNGXOR -s PRNGXOR-Results/$name
      done
    done
  done