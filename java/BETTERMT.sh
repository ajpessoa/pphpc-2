...
mkdir BETTERMT-Results

for configuration in "${configurations[@]}"
  do
    for threads in "${thread_counts[@]}"
    do
      echo "Running BETTERMT with configuration $configuration and $threads threads"
      for (( i=1; i<=$runs; i++ ))
      do
        name="BETTERMT_$configuration-$threads-$i.txt"
        sh ./pp.sh -p ../configs/$configuration.txt -ps EX -n $threads -g BETTERMT -s BETTERMT-Results/$name
      done
    done
  done