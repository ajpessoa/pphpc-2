...
for configuration in "${configurations[@]}"
  do
    ...
    do
      echo "Running PCG64 with configuration $configuration and $threads threads"
      for (( i=1; i<=$runs; i++ ))
      do
        name="PCG64_$configuration-$threads-$i.txt"
        sh ./pp.sh -p ../configs/$configuration.txt -ps EX -n $threads -g PCG64 -s PCG64-Results/$name
      done
    done
  done