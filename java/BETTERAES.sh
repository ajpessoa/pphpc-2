...
mkdir BETTERAES-Results

for configuration in "${configurations[@]}"
  do
    for threads in "${thread_counts[@]}"
    do
      echo "Running BETTERAES with configuration $configuration and $threads threads"
      for (( i=1; i<=$runs; i++ ))
      do
        name="BETTERAES_$configuration-$threads-$i.txt"
        sh ./pp.sh -p ../configs/$configuration.txt -ps EX -n $threads -g BETTERAES -s BETTERAES-Results/$name
      done
    done
  done