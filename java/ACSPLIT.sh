...
mkdir ACSPLIT-Results

for configuration in "${configurations[@]}"
  do
    for threads in "${thread_counts[@]}"
    do
      echo "Running ACSPLIT with configuration $configuration and $threads threads"
      for (( i=1; i<=$runs; i++ ))
      do
        name="ACSPLIT_$configuration-$threads-$i.txt"
        sh ./pp.sh -p ../configs/$configuration.txt -ps EX -n $threads -g ACSPLIT -s ACSPLIT-Results/$name
      done
    done
  done