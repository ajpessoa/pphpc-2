...
for configuration in "${configurations[@]}"
  do
    ...
    do
      echo "Running ACMT with configuration $configuration and $threads threads"
      for (( i=1; i<=$runs; i++ ))
      do
        name="ACMT_$configuration-$threads-$i.txt"
        sh ./pp.sh -p ../configs/$configuration.txt -ps EX -n $threads -g ACMT -s ACMT-Results/$name
      done
    done
  done