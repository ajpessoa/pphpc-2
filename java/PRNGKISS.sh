...
mkdir PRNGKISS-Results

for configuration in "${configurations[@]}"
  do
    for threads in "${thread_counts[@]}"
    do
      echo "Running PRNGKISS with configuration $configuration and $threads threads"
      for (( i=1; i<=$runs; i++ ))
      do
        name="PRNGKISS_$configuration-$threads-$i.txt"
        sh ./pp.sh -p ../configs/$configuration.txt -ps EX -n $threads -g PRNGKISS -s PRNGKISS-Results/$name
      done
    done
  done