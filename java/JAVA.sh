...
for configuration in "${configurations[@]}"
  do
    ...
    do
      echo "Running JAVA with configuration $configuration and $threads threads"
      for (( i=1; i<=$runs; i++ ))
      do
        name="JAVA_$configuration-$threads-$i.txt"
        sh ./pp.sh -p ../configs/$configuration.txt -ps EX -n $threads -g JAVA -s JAVA-Results/$name
      done
    done
  done